...
c  ------------------------------------------------------------------------
c  This program solves the elliptic partial differential equation of the 
c  form
c         _             _
c     d  |          du   |
c    --- | \kappa   ---  | = 0      i,j = 1,2
...
c                ij         ji
c
c          \kappa   = \kappa'   . g(x1,x2)
c                ij         ij
c
c          \kappa'   are constants
c                ij 
c 
c          \kappa' .\kappa'  - \kappa' .\kappa'  > 0
c                11       22         12       12
c                                          2
c                                         d g^{1/2}
c          g(x1,x2) satisfies   \kappa'   --------- + k g^{1/2} = 0
c                                     ij    dx dx
c                                             i  j
c          k  constant real number.
c
c  In this program, the function g is defined as
c                   g^{1/2} = cos( alpha1*x1 + alpha2*x2 )  for  k > 0,
c                   g^{1/2} = exp( alpha1*x1 + alpha2*x2 )  for  k < 0,
c                   g^{1/2} = c0 + c1 x1 + c2 x2            for  k = 0,
c  where  c0, c1, c2, alpha1 and alpha2 are constants, and
c          \kappa'  alpha  alpha  = k     if   k > 0
c                ij      i      j
c          \kappa'  alpha  alpha  = -k    if   k < 0.
c                ij      i      j
c  ------------------------------------------------------------------------
     ...
     &          sqlo1(nto),sqlo2(nto),sqhi1(nto),sqhi2(nto)
      ...
      dimension aa(nto,nto),bb(nto),xx(nto),uu(nto),pp(nto),value(nto)
      dimension ttt(ntime)
...
c  THE CONSTANT COEFFICIENTS \KAPPA'_{IJ}
...
c  THE BOUNDARY DATA
          do k=1,nn
            if (k.le.nsse(1)+nsse(2)+nsse(3)) then
              kode(k)=0
              value(k)=Pexact(k,sqm1(k),sqm2(k))
            else
              kode(k)=1
              value(k)=exact(sqm1(k),sqm2(k))
            endif
          enddo
c  SETUP OF THE LINEAR SYSTEM OF EQUATIONS
          do k=1,nn
            ...
            spt2=sqm2(k)
            do l=1,nn
              call binteg(99,l,spt1,spt2,smp,smg,smpv,smp1,smg1,smpv1,
     &               smp2,smg2,smpv2)
              if (kode(l).eq.0) then
                aa(k,l)=-(smpv-smg)
                bb(k)=bb(k)+smp*value(l)
              else
                aa(k,l)=-smp
                bb(k)=bb(k)+(smpv-smg)*value(l)
              endif
            enddo
            if (kode(k).eq.1) then
              bb(k)=bb(k)-0.5d0*value(k)*g12(sqm1(k),sqm2(k))
            else
              aa(k,k)=aa(k,k)+0.5d0*g12(sqm1(k),sqm2(k))
            endif
          enddo
     ...
     &                    dlog(2.0d0)/tau )
          ...
          do ii=1,19
            ...
            do jj=1,19
              ...
              psi2=0.d0
              do l=1,nn
                call binteg(1,l,xint,yint,smp,smg,smpv,smp1,smg1,smpv1,
     &               smp2,smg2,smpv2)
              ...
              enddo
              c=psi/g12(xint,yint)
              dcdx1=(psi1-c*dg12dx1(xint,yint))/g12(xint,yint)
              dcdx2=(psi2-c*dg12dx2(xint,yint))/g12(xint,yint)
...
c  END OF STEHFEST ITERATION
 ...
 236    format('#',1x,'t (1)',4x,'(x,y) (2-3)',10x,
     ...
     &   rmu(xxi,yyi,tau), rmu1(xxi,yyi,tau), rmu2(xxi,yyi,tau),
     &   (sum_mu(ii,jj)-rmu(xxi,yyi,tau))**2.d0,
     &   (sum_mu1(ii,jj)-rmu1(xxi,yyi,tau))**2.d0,
     &   (sum_mu2(ii,jj)-rmu2(xxi,yyi,tau))**2.d0,
     &   dabs(sum_mu(ii,jj)-rmu(xxi,yyi,tau)),
     &   dabs(sum_mu1(ii,jj)-rmu1(xxi,yyi,tau)),
     &   dabs(sum_mu2(ii,jj)-rmu2(xxi,yyi,tau)),
     &   dabs( (sum_mu(ii,jj)-rmu(xxi,yyi,tau)) / rmu(xxi,yyi,tau) ),
     &   dabs( (sum_mu1(ii,jj)-rmu1(xxi,yyi,tau)) / rmu1(xxi,yyi,tau) ),
     &   dabs( (sum_mu2(ii,jj)-rmu2(xxi,yyi,tau)) / rmu2(xxi,yyi,tau) )
       write(1,237)
     &   tau,xxi,yyi,sum_mu(ii,jj),sum_mu1(ii,jj),sum_mu2(ii,jj),
     &   rmu(xxi,yyi,tau), rmu1(xxi,yyi,tau), rmu2(xxi,yyi,tau),
     &   (sum_mu(ii,jj)-rmu(xxi,yyi,tau))**2.d0,
...
c  NUMERICAL INTEGRATION USING BODE'S RULE
      subroutine binteg(ints,li,spt1,spt2,smp,smg,smpv,smp1,smg1,
     &                 smpv1,smp2,smg2,smpv2)
      implicit real*8 (a-h, o-z)
      parameter (nto=1000)
      dimension qlo1(nto),qlo2(nto),qhi1(nto),qhi2(nto),
     &          qn1(nto),qn2(nto)
C      dimension psig(12),w(12)
      dimension w(5)
      ...
      do kkk=1,5
        ...
        smp = smp + ( Phi(x1,x2,spt1,spt2)/g12(x1,x2) +
     &                Phi(y1,y2,spt1,spt2)/g12(y1,y2) ) * w(kkk)
        smg = smg + ( Gamma(li,x1,x2,spt1,spt2)*g12(x1,x2) +
     &                Gamma(li,y1,y2,spt1,spt2)*g12(y1,y2) ) * w(kkk)
        smpv = smpv + ( (pkix - (v1*qn1(li)+v2*qn2(li))*g12(x1,x2)) *
     &                  Phi(x1,x2,spt1,spt2)
     &              +   (pkiy - (v1*qn1(li)+v2*qn2(li))*g12(y1,y2)) *
     &                  Phi(y1,y2,spt1,spt2) ) * w(kkk)
        if (ints.eq.1) then
        smp1 = smp1 + ( Phi1(x1,x2,spt1,spt2)/g12(x1,x2) +
     &                  Phi1(y1,y2,spt1,spt2)/g12(y1,y2) ) * w(kkk)
        smg1 = smg1 + ( Gamma1(li,x1,x2,spt1,spt2)*g12(x1,x2) +
     &                  Gamma1(li,y1,y2,spt1,spt2)*g12(y1,y2) ) * w(kkk)
        smpv1 = smpv1 + ( (pkix - (v1*qn1(li)+v2*qn2(li))*g12(x1,x2)) *
     &                    Phi1(x1,x2,spt1,spt2)
     &                +   (pkiy - (v1*qn1(li)+v2*qn2(li))*g12(y1,y2)) *
     &                    Phi1(y1,y2,spt1,spt2) ) * w(kkk)
        smp2 = smp2 + ( Phi2(x1,x2,spt1,spt2)/g12(x1,x2) +
     &                  Phi2(y1,y2,spt1,spt2)/g12(y1,y2) ) * w(kkk)
        smg2 = smg2 + ( Gamma2(li,x1,x2,spt1,spt2)*g12(x1,x2) +
     &                  Gamma2(li,y1,y2,spt1,spt2)*g12(y1,y2) ) * w(kkk)
        smpv2 = smpv2 + ( (pkix - (v1*qn1(li)+v2*qn2(li))*g12(x1,x2)) *
...
c  An exact solution
      ...
      function exact(x1,x2)
      implicit real*8 (a-h,o-z)
      external g12, psiex
      exact = psiex(x1,x2)/g12(x1,x2)
      return
      end


c  The exact solution d(\phi)/d(x1)
      function exact1(x1,x2)
      implicit real*8 (a-h,o-z)
      external g12, psiex, dg12dx1, psiex1
      vg12=g12(x1,x2)
      vpsiex=psiex(x1,x2)
      vdg12dx1=dg12dx1(x1,x2)
      vpsiex1=psiex1(x1,x2)
      exact1 = (vpsiex1*vg12-vpsiex*vdg12dx1) / (vg12*vg12)
      return
      end
c  The exact solution d(\phi)/d(x2)
      function exact2(x1,x2)
      implicit real*8 (a-h,o-z)
      external g12, psiex, dg12dx2, psiex2
      vg12=g12(x1,x2)
      vpsiex=psiex(x1,x2)
      vdg12dx2=dg12dx2(x1,x2)
      vpsiex2=psiex2(x1,x2)
      exact2 = (vpsiex2*vg12-vpsiex*vdg12dx2) / (vg12*vg12)
      return
      end
...
c  ------------------------------------------------------------------------
c  The exact solution P
      function Pexact(ki,x1,x2)
      implicit real*8 (a-h,o-z)
      external g12, psiex, Pkig, Ppsi
      call Pkig(ki,x1,x2,Pki)
      call Ppsi(ki,x1,x2,Ppsiex)
      Pexact = -Pki*psiex(x1,x2) + Ppsiex*g12(x1,x2)
      return
      end
...
c  ------------------------------------------------------------------------
c  The function P^{[\psi]}
      subroutine Ppsi(kp,x1,x2,Ppsiex)
      implicit real*8 (a-h,o-z)
      parameter (nto=1000)
      dimension xn(nto),yn(nto)
      common /moduli/ rk11,rk12,rk22
      common /normal/ xn,yn
      external psiex1, psiex2
      der1=psiex1(x1,x2)
      der2=psiex2(x1,x2)
...
c  The function \rmu(x1,x2,t)
      function rmu(x,y,t)
      implicit real*8 (a-h, o-z)
      parameter (pi=3.141592654d0)
      rmu=psiexor(x,y)/g12(x,y)*tfunc(t)
      return
      end
c  The function d(\rmu(x1,x2,t))/d(x1)
      function rmu1(x,y,t)
      implicit real*8 (a-h, o-z)
      parameter (pi=3.141592654d0)
      rmu1=( psiexor1(x,y)*g12(x,y) - psiexor(x,y)*dg12dx1(x,y) )
     &     /g12(x,y)**2.d0*tfunc(t)
      return
      end
c  The function d(\rmu(x1,x2,t))/d(x2)
      function rmu2(x,y,t)
      implicit real*8 (a-h, o-z)
      parameter (pi=3.141592654d0)
      rmu2=( psiexor2(x,y)*g12(x,y) - psiexor(x,y)*dg12dx2(x,y) )
     &     /g12(x,y)**2.d0*tfunc(t)
      return
      end

c  The function \psi(x1,x2)
      function psiex(x,y)
      implicit real*8 (a-h, o-z)
      common /laplace_constant/ rs
      psiex=sfunc(rs)*psiexor(x,y)
      return
      end
c  The function d(\psi})/d(x1)
      function psiex1(x,y)
      implicit real*8 (a-h, o-z)
      common /laplace_constant/ rs
      psiex1=sfunc(rs)*psiexor1(x,y)
      return
      end
c  The function d(\psi)/d(x2)
      function psiex2(x,y)
      implicit real*8 (a-h, o-z)
      common /laplace_constant/ rs
      psiex2=sfunc(rs)*psiexor2(x,y)
      return
      end
...
c  The derivative D(Phi)/D(a)
      ...
      common /constantD / CapD
      common /vectorV / VecV
      external FK0,FK1,Phi,Rcap,dRcap1
...
c  The derivative D^2(Phi)/D(a)^2
      ...
      common /vectorV / VecV
      common /constantCapK/ CapK
      external FK0,FK1,Phi,Phi1,Rcap,dRcap1,dRcap11
...
c  The derivative D^2(Phi)/D(a)D(b)
      ...
      common /vectorV / VecV
      common /constantCapK/ CapK
      external FK0,FK1,Phi,Phi2,Rcap,dRcap1,dRcap2,dRcap12
...
c  The derivative D^2(Phi)/D(b)^2
      ...
      common /vectorV / VecV
      common /constantCapK/ CapK
      external FK0,FK1,Phi,Phi2,Rcap,dRcap2,dRcap22
...
c  The function \psior(x1,x2)
      function psiexor(x,y)
      implicit real*8 (a-h, o-z)
      parameter (pi=3.141592654d0)
c PROB 1
      psiexor=10.d0*(dcos(-.2d0*x+.1d0*y)+dsin(-.2d0*x+.1d0*y))
      return
      end

c  The function d(\psior})/d(x1)
      function psiexor1(x,y)
      implicit real*8 (a-h, o-z)
      parameter (pi=3.141592654d0)
c PROB 1
      psiexor1=-.2d0*10.d0*(dcos(-.2d0*x+.1d0*y)-dsin(-.2d0*x+.1d0*y))
      return
      end

c  The function d(\psior)/d(x2)
      function psiexor2(x,y)
      implicit real*8 (a-h, o-z)
      parameter (pi=3.141592654d0)
c PROB 1
      psiexor2=.1d0*10.d0*(dcos(-.2d0*x+.1d0*y)-dsin(-.2d0*x+.1d0*y))
      return
      end

c  ------------------------------------------------------------------------
c  The function g^{1/2}(x1,x2)
      function g12(x,y)
      implicit real*8 (a-h, o-z)
c PROB 1
      g12=dexp(.1d0*x-.1d0*y)
      return
      end

c  The function d(g^{1/2})/d(x1)
      function dg12dx1(x,y)
      implicit real*8 (a-h, o-z)
c PROB 1
      dg12dx1=.1d0*dexp(.1d0*x-.1d0*y)
      return
      end

c  The function d(g^{1/2})/d(x2)
      function dg12dx2(x,y)
      implicit real*8 (a-h, o-z)
c PROB 1
      dg12dx2=-.1d0*dexp(.1d0*x-.1d0*y)
      return
      end
